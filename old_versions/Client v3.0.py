...
def tkintering():
    ...
    l = Label(root, width = 50, font = ('Segoe UI', 18))
    l['text'] = 'Terms and Conditions of Use for this Python ChatRoom'
    l.grid(row = 0, column = 0, columnspan = 2)

    # Text widget!
    t = Text(root, height = 25, width = 150, font = ('Segoe UI', 12), wrap = WORD)
    t['bg'] = '#FEFEFE'
    ...
    # Buttons!
    b = Button(root, text = 'I accept', command = disagree)    
    b.bind('<Button-2>', agree)
    b.bind('<Button-3>', agree)
    b.grid(row = 2, column = 0)
    
    b2 = Button(root, text = 'I do not accept', command = disagree)
...
l = Label(root, text = 'Host IP address:', font = ('Segoe UI', 12))
l.grid(row = 0, column = 0)

e = Entry(root, width = 50, font = ('Segoe UI', 12))
e.grid(row = 0, column = 1)

l2 = Label(root, text = '          Password:', justify = RIGHT, font = ('Segoe UI', 12))
l2.grid(row = 1, column = 0)

e2 = Entry(root, width = 50, font = ('Segoe UI', 12))
e2.grid(row = 1, column = 1)

b = Button(root, text = 'Enter', command = send_password)
...
def listen():
                ...
                if ']' in message and message.startswith('['):
                    ...
                    if legitMessage.startswith('/w '):
                        if username == os.getlogin():
                            ...
                            messageFrames[focus].insert(os.getlogin(), legitMessage[3:], personalColour)
                            continue
    ...
    # Yeah, that was in a try/except switch!
    except:
        pass
...
def help():
    ...
    # Label!
    l = Label(root, width = 15, font = ('Segoe UI', 24))
    l['text'] = 'Help Information'
    l.grid(row = 0, column = 0)

    # Text widget!
    t = Text(root, height = 25, width = 100, font = ('Segoe UI', 12), wrap = WORD)
    t['bg'] = '#FEFEFE'
    ...
    t.insert(END, '''Help for Python ChatRoom v3.0: 
...
class MessageFrame(Frame):
    ...
    def __init__(self, master, title):
        ...
        self.scroll.grid(row = 1, column = 3, rowspan = 3, sticky = N + S)

        # Usernames listbox!
        self.usernames = Listbox(self, width = 15, height = 20, font = ('Segoe UI', 12), yscrollcommand = self.scroll.set)
        self.usernames.bind('<MouseWheel>', self.on_mouse_wheel)
        self.usernames.grid(row = 2, column = 0)

        # Messages listbox!
        self.messages = Listbox(self, width = 60, height = 20, font = ('Segoe UI', 12), yscrollcommand = self.scroll.set)
        ...
        # Timestamps listbox!
        self.stamps = Listbox(self, width = 30, height = 20, font = ('Segoe UI', 12), yscrollcommand = self.scroll.set)
    ...
    def insert(self, username, message, colour = 'black', time = None):
        ...
        self.usernames.insert(END, username)
        self.messages.insert(END, message)
        self.stamps.insert(END, time)

        if colour == 'red':
            self.messages.itemconfig(END, {'fg': 'red'})

        elif colour == 'green':
            self.usernames.itemconfig(END, {'fg': 'green4'})
            self.messages.itemconfig(END, {'fg': 'green4'})

        elif colour == 'blue':
            self.usernames.itemconfig(END, {'fg': 'blue'})
            self.messages.itemconfig(END, {'fg': 'blue'})

        else:
            self.usernames.itemconfig(END, {'fg': colour})
            self.messages.itemconfig(END, {'fg': colour})

        self.usernames.see(END)
...
# Entry widget!
e = Entry(root, width = 92, font = ('Segoe UI', 12))
