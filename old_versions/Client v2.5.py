...
def help():
    ...
    '''
    root = Tk()
    root.title('Help for Python ChatRoom v{}. '.format(version))
    root.iconbitmap(os.getcwd() + '\\message.ico')

    # Label!
    l = Label(root, height = 1, width = 30, font = ('Segoe UI', 24))
    ...
    t.insert(END, 'Help for Python ChatRoom version {}: \n\n'.format(version))
    ...
    t.insert(END, 'Hyperlinks \n')
    t.insert(END, '  - Syntax: Click on a message. \n')
    t.insert(END, '  - Click on a message to go to that website. \n')
    t.insert(END, '  - For example, clicking on "www.aops.com" would take you to www.aops.com. ')
...
helpm.add_command(label = 'Help with Commands', command = help_thread)
...
messages.bind('<<ListboxSelect>>', browse)
