...
# Update v3.2.   
# July, 2019. 
# Added new menu stuff and pinging.
# Also, cleanup of usernames.
# v4.0 will feature cleaner code. 
...
l = tk.Label(root, text = 'Host IP address:', padx = 10, font = ('Segoe UI', 12))
l.grid(row = 0, column = 0, sticky = 'e')

e = ttk.Entry(root, width = 50, font = ('Segoe UI', 12))
e.grid(row = 0, column = 1)

l2 = tk.Label(root, text = 'Password:', padx = 10, justify = 'right', anchor = 'e', font = ('Segoe UI', 12))
...
def listen():
    ...
    try: 
        while True: 
            ...
            for message in messes:
                ...
                if message == '/ping':
                    ...
                    pingInfo['text'] = 'Ping: {}ms'.format(round(pingTime * 1000))

                    print('Ping to the server and back took {} milliseconds. '.format(pingTime * 1000))
                    
                    pingButton.grid(row = 0, column = 0, sticky = 's')
                    pingProgress.stop()
                    pingProgress.grid_forget()
                    continue
        ...
        print('''If you expected this, that's great!

If not, then please try again.
It will probably work. If it doesn't, then we genuinely don't know what's wrong.
Please contact us so that we can fix the issue.
''')
        try:
            root.destroy()
        except:
...
def send(event = None):
    ...
    if len(e.get()) != 0 and e.get() != ' ' * len(e.get()):
        ...
        if reciever != 'Lobby' and not e.get().startswith('/'):
...
def help():
    ...
    # Text widget!
    t = tk.Text(root, height = 25, width = 100, font = ('Segoe UI', 12), wrap = tk.WORD)
    t['bg'] = '#FEFEFE'
    ...
    t.insert(tk.END, '''
Help for Python ChatRoom ''' + version + ''': 
...
Hyperlinks 
  - Action: Click on a message. 
  - Click on a message to go to that website. 
  - For example, clicking on "www.aops.com" would take you to www.aops.com.
  - If the message does not contain a hyperlink, it will copy the message to your Entry widget. 

Ping
  - Action: Click the "Ping" button.
  - Servers and clients may take some time to communicate.
  - As such, you can click the "Ping" button to ping the server. 
  - It will display the amount of time taken between sending a message and recieving one back. 
''')
...
def switch_notifications(): 
    ...
    if notificationsOn:
        ...
        print('Notifications have been turned on. ')
        notificationsOn = True

def ping():
    '''
    ping()
    Ping the server and get the time!
    '''
    global pingStartTime
    pingStartTime = time.time()
    pingButton.grid_forget()
    pingProgress.grid(row = 0, column = 0, sticky = 's')
    pingProgress.start()
...
1. You forgot to close the chatroom properly last time.
     If this is the case, just run the program again and it will work. 
2. You entered the wrong IP or password.
     If this is the case, please check that you entered the IP and password correctly.
3. The server is down.
     In this case, there's no helping until the server is back on.

In any case, we are sorry for the inconvenience.
If your problem is 1. or 2. and it pursues, please email us at pyvagroup@gmail.com. ''')
    sys.exit()
...
class MessageFrame(tk.Frame):
    ...
    def __init__(self, master, title):
        ...
        self.messages.bind('<<ListboxSelect>>', self.browse)
    ...
    def insert(self, username, message, colour = 'black', time = None):
        ...
        hide = (self.usernames.get(tk.END) == username) 
        ...
        # Prevent duplicate usernaming!
        if hide:
            self.usernames.itemconfig(tk.END, {'fg': 'white'}) 
...
def focus():
    ...
    try:
        while True:
            ...
            if not focus.startswith('.!entry') and not focus.endswith('.!listbox2'):
                e.focus_set()
    except:
        return
...
on = tk.IntVar(value = 1)
off = tk.IntVar(value = 0)
...
title = tk.Label(root, text = 'Python ChatRoom', width = 30, font = ('Segoe UI', 24))
title['text'] = title['text'] + ' of ' + invitation
title.grid(row = 0, column = 1, columnspan = 1, sticky = 'nwse')

info = tk.Label(root, text = 'Local address: ' + localIP + '\nServer address: ' + IP_address,
             width = 30, font = ('Segoe UI', 12))
info.configure(justify = 'right', anchor = 'w')
info.grid(row = 0, column = 2, sticky = 'w')

# Ping information!
pingInfo = tk.Label(root, text = 'Ping! ', width = 15, font = ('Segoe UI', 12))
pingInfo.grid(row = 0, column = 0, sticky = 'n')

pingButton = ttk.Button(root, text = 'Ping', command = ping)
pingButton.grid(row = 0, column = 0, sticky = 's')

pingProgress = ttk.Progressbar(root, length = 120, mode = 'indeterminate')
...
settings.add_checkbutton(label = 'Notifications', onvalue = 1, offvalue = 0, variable = on, command = switch_notifications)
...
menu.add_cascade(label = 'Options', menu = settings)
...
print('\nProgram stopped at ' + str(datetime.now()) + '. ')
while True:
    pass
